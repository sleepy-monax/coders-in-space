...
def connect_to_player(player_id, remote_IP='127.0.0.1', verbose=False):
    ...
    try:
        if verbose: print 'binding on %s:%d to receive orders from player %d...' % (local_IP, local_port, player_id)

        socket_in.bind((local_IP, local_port))
    except:
        local_port = 42000 + 100 + (3-player_id)
        if verbose: print '   referee detected, binding instead on %s:%d...' % (local_IP, local_port)
    ...
    while not connected:
        try:
            ...
            connected = True
            if verbose: print '   done -> now sending orders to player %d on %s:%d' % (player_id, remote_IP, remote_port)
        except:
            if verbose and not msg_shown:
