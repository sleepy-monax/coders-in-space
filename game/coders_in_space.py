...
def get_ai_input(g_data, player_name):
    ...
    neural_network = g_data['players'][player_name]['network']
    outputs = []

    for ship in g_data['ships']:
        ship_data = g_data['ships'][ship]
        ship_neural_input = ship_to_neural_input(g_data, player_name, ship)
        ...
        if ship_data['owner'] == player_name:
            ...
            # Get nearby ships.
            nearby_ships = get_nearby_ship(g_data, ship, 10)[:3]
            for nearby_ship in nearby_ships:
                neural_input.extend(ship_to_neural_input(g_data, player_name, nearby_ship))

            # Get Neural output.
            neural_output = compute_neural_network(neural_network.copy(), neural_input)

            outputs.append(neural_output_to_game_input(neural_output, ship, g_data))
...
def ship_to_neural_input(g_data, player_name, ship_name):
    """
    ...
    # Get the heal.
    ship_heal_data = ship['heal_points'] / g_data['model_ship'][ship_type]['max_heal']
    ...
    ship_position_data = [ship['position'][0] / g_data['board_size'][0], ship['position'][1] / g_data['board_size'][1], ship_heal_data]
...
def neural_output_to_game_input(neural_ouput, ship_name, g_data):
    """
...
def compute_neural_network(neural_network, neural_input):
    ...
    for layer in neural_network:
        for node in neural_network[layer]:
            if layer == 0 and node < len(neural_input):
        ...
        for network_a in range(batch_size):
            for network_b in range(batch_size):
                if network_a > network_b:
                    ...
                    g_data = play_game('random', ('bot%d' % network_a, 'bot%d' % network_b), screen_size = (190, 50), no_gui = False, no_splash = True, max_rounds_count = 10)

                    if len(g_data['winners']) == 1:
                        winner_id = int(g_data['winners'][0].replace('bot', ''))
                    ...
                    elif len(g_data['winners']) == 2:
                        neurals_networks[network_a]['fitness'].append(0)
                        neurals_networks[network_b]['fitness'].append(0)

                    # neurals_networks[network_a]['fitness'].append(g_data['players']['bot%d' % network_a]['fitness'])
                    # neurals_networks[network_b]['fitness'].append(g_data['players']['bot%d' % network_b]['fitness'])

                    print 'b:%d f:%d // b:%d f:%d' % (network_a, g_data['players']['bot%d' % network_a]['fitness'], network_b, g_data['players']['bot%d' % network_b]['fitness'])
